...
public class SimpleListItemAdapter extends BaseAdapter {
	...
	public SimpleListItemAdapter(Context context,
			List<? extends SimpleListItem> items) {
		this.context = context;
		this.items = items;
	}

	/**
	 * Allows updating of the full data list underlying this adapter, replacing
	 * all items
	 * 
	 * @param newItems
	 *            The new list of simple list items to display
	 */
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null || !(convertView instanceof SimpleListItemView)) {
			filterItemView = SimpleListItemView.build(context);
		} else {
	...
	/**
	 * Represents a very simple list item that only contains a single string to
	 * show in the list. Use wrapStringsList to wrap an existing list of strings
	 * into a list of {@link SimpleListItem}s.
	 * 
	 * @author Eric Kok
	 */
	public static class SimpleStringItem implements SimpleListItem {
		...
		/**
		 * Wraps a simple string of strings into a list of SimpleStringItem to
		 * add as data to a {@link SimpleListItemAdapter}
		 * 
		 * @param strings
		 *            A list of string
		 * @return A list of SimpleStringItem objects representing the input
		 *         strings
		 */
		public static List<SimpleStringItem> wrapStringsList(
				List<String> strings) {
