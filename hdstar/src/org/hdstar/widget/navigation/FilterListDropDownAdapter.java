...
/**
 * List adapter that holds filter items, that is, servers, view types and labels
 * and is displayed as content to a Spinner instead of a ListView.
 * 
 * @author Eric Kok
 */
public class FilterListDropDownAdapter extends FilterListAdapter {

	public FilterListDropDownAdapter(Context context) {
		super(context);
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (navigationSelectionView == null) {
			navigationSelectionView = NavigationSelectionView.build(context);
		}
	...
	public void updateCurrentFilter(NavigationFilter currentFilter) {
		...
		if (navigationSelectionView != null)
			navigationSelectionView
					.bind(this.currentServer, this.currentFilter);
	}

	public void updateCurrentServer(RemoteSetting setting) {
		this.currentServer = setting.name;
		if (navigationSelectionView != null)
			navigationSelectionView
					.bind(this.currentServer, this.currentFilter);
	}
